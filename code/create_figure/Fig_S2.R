...
# Plot
plot_samples <- ggplot(long_df_samples, aes(x = Sample_Label, y = Relative_Abundance)) +
  geom_boxplot(fill = "#D9CCE3", outlier.size = 0.5) +
  theme_minimal(base_size = 13) +
  labs(x = "Sample (Month-Day-Depth-Oxygen)", y = "Relative Abundance of MAGs") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 9))

# Save
ggsave("FigS2.pdf", plot = plot_samples, width = 12, height = 5, dpi = 400)
